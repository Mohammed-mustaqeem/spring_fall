...
const Header = () => {
  ...
  const navItems = [
    ...
    { name: 'Uniportal', path: 'https://springfall-usa.vercel.app/' },
  ];
  ...
  return (
    ...
    >
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                target={item.path === 'https://springfall-usa.vercel.app/' ? '_blank' : '_self'}
      ...
      <div
        className={`lg:hidden fixed inset-0 z-50 transform transition-transform duration-300 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
